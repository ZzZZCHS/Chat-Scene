...
split = 'val'
all_objects = json.load(open('annotations/all_objects_with_full_desc.json'))
    ...
    for o in v['object_list']:
        if o['description'] is not None:
            new_annos.append({
                'scene_id': k,
                'obj_id': o['object_id'],
                'prompt': random.choice(templates).replace('<id>', f"{o['object_id']:02}"),
                'caption': o['description'],
                'related_ids': [o['object_id']]
            })
print(len(new_annos))

with open(f'annotations/scannet_{split}_stage2_caption.json', 'w') as f:
